...
func GetKeycloakProvisionCommand(cr *orgv1.CheCluster) (command string) {
	...
	if cheFlavor == "che" {
		// java home fix....
		command = "cd ${HOME} && export JAVA_TOOL_OPTIONS=-Duser.home=. && " + createRealmClientUserCommand
	}
	return command
}
...
func GetOpenShiftIdentityProviderProvisionCommand(cr *orgv1.CheCluster, oAuthClientName string, oauthSecret string, isOpenShift4 bool) (command string, err error) {
	...
	*/
	...
	if cheFlavor == "che" {
		// todo
		command = "cd ${HOME} && export JAVA_TOOL_OPTIONS=-Duser.home=. && " + command
	}
	return command, nil
}
...
func GetDeleteOpenShiftIdentityProviderProvisionCommand(cr *orgv1.CheCluster, isOpenShift4 bool) (command string) {
	...
	if cheFlavor == "che" {
		// todo
		command = "cd ${HOME} && export JAVA_TOOL_OPTIONS=-Duser.home=. && " + deleteOpenShiftIdentityProviderCommand
	}
	return command
}
